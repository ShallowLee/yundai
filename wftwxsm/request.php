...
Class Request{
    ...
    public function submitOrderInfo(){
		...
		$this->reqHandler->setParameter('op_shop_id','1234');
		...
		$this->reqHandler->setParameter('limit_credit_pay','1');     //限定用户使用时能否使用信用卡，值为1，禁用信用卡，值为0或者不传此参数则不禁用
		//this->reqHandler->setParameter('goods_tag','1fds');
        //$this->reqHandler->setParameter('groupno','8111100093');
        //通知地址，必填项，接收平台通知的URL，需给绝对路径，255字符内
        ...
        if($this->pay->call()){
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    //echo json_encode(array('code_img_url'=>$this->resHandler->getParameter('code_img_url'), 'code_url'=>$this->resHandler->getParameter('code_url'), 'code_status'=>$this->resHandler->getParameter('code_status')));
                    //exit();
					$status = "0";
    ...
    public function callback(){
        ...
        if($this->resHandler->isTenpaySign()){
            if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
				...
				$ch=curl_init(); 
				curl_setopt($ch,CURLOPT_URL,"http://pay08.hzit.com/Payapi_WftWxSM_NotifyUrl.html");
				//1=设置头文件的信息作为数据流输出
