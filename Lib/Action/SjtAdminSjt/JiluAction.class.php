...
class JiluAction extends Action{
	...
	public function wyjl(){
          ...
          $typepay = $this->_get("typepay");         //交易类型
          $shbh = $this->_get("shbh");               //商户编号
          $TransID = $this->_get("TransID");         //订单号
	...
	public function Tongji() {
		...
		$Order = M("Order");
		$daynum = $Order->where($wherestr)->count();                   //总笔数
        $daymoney = $Order->where($wherestr)->sum("trademoney");       //总金额
		if($daymoney === null){
			$daymoney = 0;
		}
        $daysjmoney = $Order->where($wherestr)->sum("OrderMoney");     //实收总金额
		if($daysjmoney === null){
			$daysjmoney = 0;
		}
        $daylrmoney = $Order->where($wherestr)->sum("sxfmoney");       //实收总总利润
		if($daylrmoney === null){
			$daylrmoney = 0;
		}
	...
	public function XTongji() {
		...
		//$jssqtklr = $sqtkbsmoney * 0.02;   //提款中给我们的技术服务费 暂定2%
		if($sqtkbsmoney === null){
			$sqtkbsmoney = 0;
		}
        ...
        $jshjlr = $hjlr * 0.01;                                       //给我们的技术服务费 暂定1%
