...
class ShangHuAction extends Action{
	...
	public function xiane(){  
		...
		$UserID = $this->_get("UserID");
        $User = M("User");
        $list = $User->where("id=".$UserID)->select();
    ...
    //修改商户每日交易限额
	public function xianeedit(){   
           $User = M("User");
           $users["dailylimit"] = $this->_post("dailylimit");
           $UserID = $this->_post("UserID");
           $User->where("id = ".$UserID)->save($users);
           echo "ok";
    }
