<?php
     class AlipaywapAction extends PayAction{
         ...
         public function Post(){
			...
			//die("PayName:".$this->PayName);
            $this->check();
            $this->Orderadd();
            $this->_Merchant_url = "http://".C("WEB_URL")."/Payapi_Alipaywap_MerChantUrl.html";    //商户通知地址
            $this->_Return_url = "http://".C("WEB_URL")."/Payapi_Alipaywap_ReturnUrl.html";        //用户通知地址
	  ...
	  public function ReturnUrl(){
	      	...
	      	if($verify_result) {
	      		...
	      		if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
	      			...
	      			echo "fail:trade_status=".$trade_status;
	      		}
	      	} else {
	      		//验证失败
	      		echo "ReturnUrl验证失败";
