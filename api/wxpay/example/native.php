...
$input->SetNotify_url("http://pay08.hzit.com/api/wxpay/example/notify.php");
$input->SetTrade_type("NATIVE");
$input->SetProduct_id("123456");
$result = $notify->GetPayUrl($input);
$url2 = $result["code_url"];
//echo "err_code = ".$result["err_code"]."  err_code_des: ".$result["err_code_des"];
?>
<html>
<head>
...
<body>
     ...
     <div id="main">
        ...
        <div id="content">
                            <div>
                            <img  alt="扫码支付" src="http://pay08.hzit.com/api/wxpay/example/qrcode.php?data=<?php echo urlencode($url2);?>" style="width:300px;height:300px;"/>
                            </div>
                    </div>
        <div id="QRmsg"><div id="msgContent" class="qr_default"><p>请使用微信扫描<br/>二维码完成支付</p></div></div>
    </div>
...
var out_sn = '<?php echo $_GET['out_trade_no'];?>';
 var f= setInterval("myInterval()",4000);  //1000为1秒钟  即4秒轮询一次后台看看支付成功了没
       ...
       {    
			 ...
			 $.ajax({
			    url:'http://pay08.hzit.com/Payapi_Wxdemo_MerChantUrl.html',
			    type:'POST',   //不行就试试GET
			    async:true,    //是否异步
			    data:{
			        out_trade_no:out_sn,
			    ...
			    success:function(res,textStatus,jqXHR){
						  ...
						  if(res==1)
						  {
							  clearInterval(f);  //清除定时器 跳转到同步回调页面
							  location.href='http://pay08.hzit.com/Payapi_Wxdemo_ReturnUrl.html?sp_billno=<?php echo $_GET['out_trade_no'];?>&trade_status=SUCCESS';
							  return false;
						  }else
						  {
							   console.log('输出'+res)
