...
//    exit();
    $data['notifyResp'] =  array_to_object($llpayNotify->notifyResp);
    $data['result'] =  $llpayNotify->result;
    $data['result_pay'] =  $llpayNotify->notifyResp['result_pay'];
    $data['llpay_config'] =  $llpayNotify->llpay_config;
//    $gateWary="http://yundai.itcitylife.com/Payapi_Llpay_Pay.html";
//    $http_post = http_post($gateWary,$llpayNotify->notifyResp['money_order']);

	if($result_pay == "SUCCESS"){
        $data = json_decode(file_get_contents("php://input"));
        $http_post = http_post($notify,$data);
